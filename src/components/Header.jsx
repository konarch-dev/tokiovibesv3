    ...
    <>
        ...
        <div className="container header-container">
          ...
          <div className="header-left">
            <nav className="desktop-only header-nav-links">
              <Link to="/">Home</Link>
              <Link to="/studio" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                My Studio
                <span style={{ 
                  fontSize: '8px', 
                  background: '#970C0F', 
          ...
          <div className="header-right">
            ...
            <div className="header-icons">
              {isSearchOpen ? (
                <form onSubmit={handleSearchSubmit} className="search-form desktop-only" style={{ display: 'flex', alignItems: 'center', background: '#f5f5f5', borderRadius: '20px', padding: '2px 10px' }}>
                  <input
                    type="text"
                    placeholder="Search..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    style={{ border: 'none', background: 'transparent', outline: 'none', padding: '5px', width: '150px' }}
                    autoFocus
                  />
                  <button type="button" onClick={() => setIsSearchOpen(false)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '0 5px' }}>
                ...
                <svg className="icon desktop-only" viewBox="0 0 24 24" onClick={() => setIsSearchOpen(true)} style={{ cursor: 'pointer' }}>
              ...
              <div style={{ color: 'inherit', cursor: 'pointer' }} onClick={() => setIsMiniCartOpen(true)}>
      ...
      <div className={`mobile-sidebar ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        <nav className="sidebar-nav-links">
          ...
          <Link to="/studio" onClick={() => setIsMobileMenuOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            My Studio
            <span style={{ 
              fontSize: '8px', 
              background: '#970C0F', 
